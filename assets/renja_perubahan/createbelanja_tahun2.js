function listBelanja_2(clue){
    ...
    if ($("#isEdit_2").val() != "1") {
        ...
        td10.appendChild(generateTeksHapus_2(inIndex_2));
    ...
    if (inEdit != "1") {
      ...
      var volume = $("#volume_2").val().replace(/\B(?=(\d{3})+(?!\d))/g, ".");
      var nominal = $("#nominal_satuan_2").val().replace(/\B(?=(\d{3})+(?!\d))/g, ".");

      $("#uraian_1").val($("#uraian_2").val());
      $("#det_uraian_1").val($("#det_uraian_2").val());
      $("#volume_1").val('0');
      $("#satuan_1").val($("#satuan_2").val());
      $("#nominal_satuan_1").val(nominal);
    }
    listBelanja_1('jns');
    document.getElementById("satuan_1").value = '';
    $("#satuan_1").trigger("chosen:updated");
    prepare_chosen();
  ...
  function generateTeksHapus_2(index){
    ...
    teks.id = "hapusrow";
    teks.setAttribute('onclick','hapusrow_2('+index+')');
  ...
  function hapusrow_2(index){
    if(index >= 0){
      hapusrow_1($("input[name='r_kd_jenis_belanja_1["+ index +"]']")[0]);
      index = $("input[name='r_kd_jenis_belanja_2["+ index +"]']")[0];
    }
