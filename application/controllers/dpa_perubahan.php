...
class Dpa_perubahan extends CI_Controller
{
	...
	public function __construct()
	{
        ...
        $this->load->model(array('m_dpa_perubahan','m_skpd','m_lov','m_urusan', 'm_bidang', 'm_program', 'm_kegiatan', 'm_template_cetak', 'm_kendali',
	...
	function dpa_skpd(){
		...
		if (empty($data['program'])) {
			if ($kode_unit != $id_skpd) {
				redirect('home/kosong/DPA_Perubahan');
			}
		}
	...
	function get_rka(){
		...
		if ($result) {
			$msg = array('success' => '1', 'msg' => 'RKA Perubahan berhasil diambil.');
			echo json_encode($msg);
		}else{
			$msg = array('success' => '0', 'msg' => 'ERROR! RKA Perubahan gagal diambil, mohon menghubungi administrator.');
			echo json_encode($msg);
		}
	...
	function cru_program_skpd(){
		...
		if (!empty($id)) {
			...
			$data['rencana_aksi'] = $this->m_dpa_perubahan->get_rencana_aksi($id);
			$data['anggaran_rencana_aksi'] = $this->m_dpa_perubahan->get_sum_anggaran_rencana_aksi($id);
		...
		//print_r($data['kategori_indikator']);
		//exit();
	...
	function save_program_dpa(){
		...
		if (!empty($id)) {
			$result = $this->m_dpa_perubahan->edit_program_skpd($data, $id, $indikator, $id_indikator_program, $satuan_target, $status_indikator, $kategori_indikator, $target, $id_for_rencana);
		}else{
			$result = $this->m_dpa_perubahan->add_program_skpd($data, $indikator, $satuan_target, $status_indikator, $kategori_indikator, $target, $id_for_rencana);
		}
	...
	function cru_kegiatan_skpd(){
		...
		//get data triwulan dari table tx_dpa_prog_keg_perubahan bedasarkan id
		...
		$data['detil_kegiatan_1'] = $this->m_dpa_perubahan->get_dpa_belanja_per_tahun($id, $this->session->userdata('t_anggaran_aktif'));
		$data['kd_kegiatan'] = form_dropdown('kd_kegiatan', $kd_kegiatan, $kd_kegiatan_edit, 'data-placeholder="Pilih Kegiatan" class="common chosen-select" id="kd_kegiatan"');
	...
	function save_kegiatan(){
		...
		if (!empty($id)) {
			$result = $this->m_dpa_perubahan->edit_kegiatan_skpd($data, $id, $indikator, $id_indikator_kegiatan, $satuan_target, $status_indikator, $kategori_indikator, $target, $dataKegiatan1, $id_for_rencana);
		}else{
			$result = $this->m_dpa_perubahan->add_kegiatan_skpd($data, $indikator, $satuan_target, $status_indikator, $kategori_indikator, $target, $dataKegiatan1, $id_for_rencana);
		}
		...
		for ($i=1; $i < 13; $i++) {
			...
			if ($this->db->query("select * from tx_dpa_prog_keg_triwulan_perubahan where id_dpa_prog_keg='".$id."' and id_triwulan='".$i."'")->num_rows()>0) {
				$this->db->query("update tx_dpa_prog_keg_triwulan_perubahan set anggaran='".$nominal."' where id_dpa_prog_keg='".$id."' and id_triwulan='".$i."'");
			}else {
				$this->db->query("insert tx_dpa_prog_keg_triwulan_perubahan(id_dpa_prog_keg,id_triwulan,anggaran) value ('".$id."','".$i."',".$nominal.")");
			}
		}
	...
	function preview_kegiatan_dpa(){
		...
		if (!empty($result)) {
			...
			$data['tahun1'] = $this->m_dpa_perubahan->get_dpa_belanja_per_tahun($id,  $this->m_settings->get_tahun_anggaran_aktif_db());
	...
	$this->auth->restrict();
		...
		$data_rka = $this->m_dpa_perubahan->get_rka_by_id($id_rka);
		if(empty($data_rka)) {
			//cek bank baru
		...
		if(empty($id_rka)) {
			//insert
			$ret = $this->m_dpa_perubahan->simpan_rka($data_rka);
			//echo $this->db->last_query();
		} else {
			//update
			$$ret = $this->m_dpa_perubahan->update_rka($data_rka, $id_rka,'table_rka', 'primary_rka');
			//echo $this->db->last_query();
		}
		if ($ret === FALSE){
            $this->session->set_userdata('msg_typ','err');
	...
	/*function save_rka()
	{
		...
		$cek_rka = $this->m_dpa_perubahan->get_data(array('id_rka'=>$id_rka),'table_rka');
	...
	function load_rka()
	{
		...
		$rka = $this->m_dpa_perubahan->get_data_table($search, $start, $length, $order["0"]);
		$alldata = $this->m_dpa_perubahan->count_data_table($search, $start, $length, $order["0"]);
	...
	function edit_rka($id_rka = NULL)
	{
        ...
        if (!empty($id_rka)) {
            ...
            $result = $this->m_dpa_perubahan->get_data_with_rincian($id_rka,'table_rka');
			if (empty($result)) {
				$this->session->set_userdata('msg_typ','err');
	...
	function delete_rka()
	{
        ...
        $result = $this->m_dpa_perubahan->delete_rka($id);
        if ($result) {
	...
	private function cetak_func221($cetak=FALSE,$ta, $id_kegiatan){
		if (!$cetak) {
		...
		$data['tahun1'] =$this->m_dpa_perubahan->get_renja_belanja_per_tahun221($ta,$id_kegiatan);

		//  print_r($data['tahun1']);
	 // exit();
		$data['keluaran'] =$this->m_dpa_perubahan->get_indikator_keluaran($ta, $id_kegiatan);
		$data['capaian'] =$this->m_dpa_perubahan->get_indikator_capaian($id_kegiatan,$ta);
		$data['nominal'] =$this->m_dpa_perubahan->get_nominal_dpa($id_kegiatan,$ta);

		// print_r($data['nominal']);
		// exit();

		$result = $this->load->view("dpa_perubahan/cetak/cetak_form_221",$data, TRUE);
...
function cetak_kegiatan($id_kegiatan){
	...
	// print_r($id_kegiatan);
	// exit();
	$ta = $this->m_settings->get_tahun_anggaran_aktif_db();
			$protocol = stripos($_SERVER['SERVER_PROTOCOL'],'https') === true ? 'https://' : 'http://';
	$header = $this->m_template_cetak->get_value("GAMBAR");
	...
	$data['cetak'] = $this->cetak_func221(TRUE,$ta,$id_kegiatan);
	...
	function view_rencana_aksi($view, $uniq_id, $id_dpa, $id, $kd_status){
		...
		foreach ($view as $row_rcn) {
			if ($rcn_bln != $row_rcn->bulan) {
				...
				$rcn_per_bulan = $this->db->query("SELECT COUNT(bulan) AS tot_row, SUM(bobot) AS sum_bot, SUM(anggaran) AS sum_ang FROM tx_dpa_rencana_aksi_perubahan WHERE bulan = '".$rcn_bln."' AND id <> '".$id."' AND (id_dpa_prog_keg = '".$uniq_id."' OR id_dpa_prog_keg = '".$id_dpa."')")->row();
...
if ($kd_status == 1) {
	...
	function save_rencana_aksi(){
		...
		if (!empty($id)) {
			$this->m_dpa_perubahan->edit_rencana_aksi($id, $data);	
		}else{
			$this->m_dpa_perubahan->add_rencana_aksi($data);
		}

		$view = $this->m_dpa_perubahan->get_rencana_aksi($data['id_dpa_prog_keg'], $id_dpa);
	...
	function delete_rencana_aksi(){
		...
		$this->m_dpa_perubahan->delete_rencana_aksi($id);

		$view = $this->m_dpa_perubahan->get_rencana_aksi($uniq_id, $id_dpa);
	...
	function edit_rencana_aksi(){
		...
		$view = $this->m_dpa_perubahan->get_rencana_aksi($uniq_id, $id_dpa, $id, TRUE);
	...
	function one_rencana_aksi(){
		...
		$view = $this->m_dpa_perubahan->get_rencana_aksi(NULL, NULL, $id);
	...
	function get_sum_anggaran_per_bulan(){
		...
		$uniq_id = $this->input->post('id_dpa_prog_keg');
		$id_dpa = $this->input->post('id_dpa');
		$total = $this->m_dpa_perubahan->get_sum_anggaran_rencana_aksi($uniq_id, $id_dpa);
	...
	function cetak_aksi($id_skpd,$t_anggaran,$aksi,$id=null){
		...
		if ($aksi == "rencana_prog") {
			$JenisLaporan = 'rencana-aksi-program-perubahan';
			$dir = "rencana_aksi_program";
			$data['aksi'] = $this->m_dpa_perubahan->get_all_program($id_skpd, $t_anggaran);
		}elseif ($aksi == "capaian_prog") {
			$JenisLaporan = 'capaian-aksi-program-perubahan';
			$dir = "capaian_aksi_program";
			$data['aksi'] = $this->m_dpa_perubahan->get_all_program($id_skpd, $t_anggaran);
		}elseif ($aksi == "rencana_keg") {
			$JenisLaporan = 'rencana-aksi-kegiatan-perubahan';
			$dir = "rencana_aksi_kegiatan";
			$data['aksi'] = $this->m_dpa_perubahan->get_one_program($id);
		}elseif ($aksi == "capaian_keg") {
			$JenisLaporan = 'capaian-aksi-kegiatan-perubahan';
		...
		$result = $this->load->view("dpa_perubahan/cetak/".$dir, $temp, TRUE);
		 // print_r($result);exit();
