...
class M_ppas extends CI_Model
{
	...
	function count_jendela_kontrol($id_skpd,$ta){
		...
		$query = "SELECT
					...
					FROM
						t_ppas_prog_keg
					WHERE
						(id  IN (SELECT id FROM t_ppas_prog_keg WHERE (nominal > 0 OR nominal_thndpn > 0) AND is_prog_or_keg = '".$this->is_kegiatan."' AND tahun = '".$ta."' AND t_ppas_prog_keg.id_skpd)
						OR id  IN (SELECT id FROM t_ppas_prog_keg WHERE (nominal > 0 OR nominal_thndpn > 0) AND is_prog_or_keg = '".$this->is_program."' AND tahun = '".$ta."' AND t_ppas_prog_keg.id_skpd)) 
	...
	function get_all_program($id_skpd,$ta){
		if ($this->session->userdata("id_skpd") > 100) {
			...
			WHERE (tref.nom1 > 0 OR tref.nom2 > 0)
			ORDER BY `kd_urusan` asc, `kd_bidang` asc, `kd_program` asc";
	...
	function get_rka_belanja_per_tahun221($tahun_pilihan, $is_tahun, $idK){
		//------- query by deesudi
			$query = $this->db->query("SELECT id ,tahun,
							...
							SELECT id FROM `t_ppas_prog_keg` WHERE Kd_Urusan = kode_urusan  AND Kd_Bidang = kode_bidang AND kd_program = kode_program AND is_prog_or_keg ='1'
							) AS id_program,(
