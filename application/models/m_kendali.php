...
class M_kendali extends CI_Model
{
	...
	var $history_belanja = 'tx_dpa_history';
...
//-----------------------------------------------KENDALI BELANJA----------------------------------------------
		...
		function get_program_dpa_4_cetak($id_skpd,$tahun,$kd_urusan,$kd_bidang)
	    {
	    	$query = "SELECT pro.*
				FROM
					(SELECT * FROM tx_dpa_prog_keg WHERE is_prog_or_keg=1) AS pro
...
//----------------------------------------------KENDALI RENJA----------------------------------------------------
		function get_program_rka($id_skpd,$tahun)
		{
			$query = "SELECT pro.*,
					...
					AND keg.tahun = ?
					GROUP BY pro.id
					HAVING SUM(keg.nominal)>0 AND  SUM(keg.nomrenja) > 0
		...
		function get_kegiatan_rka_4_cetak($id_program,$tahun){
			$query = "
						SELECT a.`id`, a.`id_skpd`,a.`tahun`, a.`kd_urusan`, a.`kd_bidang`, a.`kd_program`, a.`kd_kegiatan`, a.`parent`,
							   ...
							   a.kesesuaian,a.hasil_kendali,a.tindak_lanjut,a.hasil_tl,a.id_status
						FROM tx_rka_prog_keg a
						LEFT JOIN t_renja_prog_keg AS b ON a.`kd_urusan`=b.`kd_urusan`
												  AND a.`kd_bidang`=b.`kd_bidang`
												  AND a.`kd_program`=b.`kd_program`
												  AND a.`kd_kegiatan`=b.`kd_kegiatan`
												  AND a.`is_prog_or_keg`=b.`is_prog_or_keg`
	...
	function get_history($id, $tahun){
			$query = "SELECT a.*,b.status
		              FROM tx_rka_history a
					  INNER JOIN m_status_tx b ON a.id_status=b.id
					  INNER JOIN tx_rka_prog_keg c ON a.id_rka = c.id
					  WHERE a.id_rka = ?
					  AND c.tahun = ?
				     ";
			$data = array($id, $tahun);
