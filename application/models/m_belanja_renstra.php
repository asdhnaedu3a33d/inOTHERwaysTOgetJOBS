...
class m_belanja_renstra extends CI_Model 
{
	var $table = 't_belanja_renstra';	
	var $table_detil = 't_belanja_renstra_detil';	
	...
	var $primary_groups = 'id_belanja_renstra';
	...
	function delete($id, $id_group){		
		$this->db->where('id_belanja_renstra', $id);
		//$this->db->where("id_group", $id_group);
		$result = $this->db->delete('t_belanja_renstra'); 
		return $result;
	}
	function get_data_belanja($id_belanja){
		$this->db->from($this->table);	
		$this->db->where("id_belanja_renstra", $id_belanja);		
		$result = $this->db->get();
		return $result->row();
	}
	...
	{
		$sql = "select * from t_belanja_renstra 
				where id_belanja_renstra = ?";
	...
	function get_data_table($search, $start, $length, $order){		
		...
		$sql = "Select * from (
				select concat(kode_jenis_belanja,'.',kode_kategori_belanja,'.',kode_subkategori_belanja,'.',kode_belanja) as kode_belanja
					   ,m_belanja.belanja,t_belanja_renstra.uraian,t_belanja_renstra.nominal
			           ,t_belanja_renstra.tahun,created_by,t_belanja_renstra.id_belanja_renstra
				from t_belanja_renstra INNER join m_belanja
				on  t_belanja_renstra.kode_jenis_belanja = m_belanja.kd_jenis_belanja 
				AND t_belanja_renstra.kode_kategori_belanja = m_belanja.kd_kategori_belanja 
				AND t_belanja_renstra.kode_subkategori_belanja = m_belanja.kd_subkategori_belanja
				AND t_belanja_renstra.kode_belanja = m_belanja.kd_belanja
				) as t_ref
				Where (t_ref.kode_belanja LIKE '%".$search['value']."%' 
						OR t_ref.belanja LIKE '%".$search['value']."%' 
					  )AND t_ref.tahun = ".$ta."
				       AND t_ref.created_by = '".$user_skpd."'
				limit ".$start.",".$length."";
	...
	//DIGANTI
	function count_data_table($search){
