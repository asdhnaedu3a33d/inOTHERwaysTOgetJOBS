...
<script type="text/javascript">
	$(document).ready(function(){
		$(".tbh_kegiatan").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas_perubahan/cru_kegiatan_skpd"); ?>',
				data: {id_program: idr},
		...
		$(".remove-kegiatan").click(function(){
			if (confirm('Apakah anda yakin untuk menghapus data kegiatan ini?')) {
				...
				$.ajax({
					type: "POST",
					url: '<?php echo site_url("ppas_perubahan/delete_kegiatan"); ?>',
		...
		$("#kegiatan td.td-click").click(function(){
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas_perubahan/preview_kegiatan_renja"); ?>',
			...
			$.ajax({
				type: "POST",
				url: '<?php echo site_url("ppas_perubahan/preview_periode_221"); ?>',
				data: {id:$(this).attr("idK")},
				success: function(msg){
					if (msg!="") {
						$.facebox(msg);
	...
	<?php
		if (!empty($kegiatan)) {
			...
			foreach ($kegiatan as $row) {
				$i++;
				$style='';
				$indikator_keg = $this->m_ppas_perubahan->get_indikator_prog_keg_perubahan($row->id);
				$id= $row->id;
				$parent = $row->parent;
