 	...
 	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    echo "Edit Data PPAS Perubahan";
			} else{
			    echo "Input Data PPAS Perubahan";
			}
		?>
		</h3>
 	</header>
 	<div class="module_content">
      <form method="post" name='input_rka' id='input_rka' action="<?php echo site_url('ppas_perubahan/save_rka')?>" enctype="multipart/form-data" >
        ...
        <table id="rka_input" class="fcari" width="100%">
          ...
          <div class="submit_link">
    			...
    			<input type="button" value="Keluar" onclick="window.location.href='<?php echo site_url('rka_perubahan'); ?>'" />
              	...
              	if(!empty($call_from) && strpos($call_from, 'ppas_perubahan/cru_rka') == FALSE)
              		echo $call_from;
              	else
              		echo site_url('ppas_perubahan/');
              ?>'"/>
  		    </div>
   		</form>
   	</div>
