...
<?php
	foreach ($urusan as $row_urusan) {
			...
			SELECT t.*,b.Nm_Bidang as nama_bidang from (
				...
				FROM
					(SELECT a.`id`,
					 ...
					 FROM t_ppas_prog_keg_perubahan a
		...
		$program = $this->m_rka_perubahan->get_program_skpd_4_cetak_vdee($id_skpd,$ta,$row_urusan->kd_urusan,$row_bidang->kd_bidang);

	foreach ($program as $row) {
		$result = $this->m_rka_perubahan->get_kegiatan_skpd_4_cetak($row->id_skpd, $row->tahun, $row->kd_urusan, $row->kd_bidang, $row->kd_program);
		...
		$indikator_program = $this->m_rka_perubahan->get_indikator_prog_keg_vdee($get_id_rka, FALSE, TRUE);
...
<?php
		...
		foreach ($kegiatan as $row) {
			$indikator_kegiatan = $this->m_rka_perubahan->get_indikator_prog_keg_vdee($row->id, FALSE, TRUE);
