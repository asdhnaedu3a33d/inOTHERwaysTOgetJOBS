 	...
 	<header>
		...
		<?php
			if (isset($isEdit) && $isEdit){
			    ...
			    echo "Input Data PPAS";
			}
		?>
		</h3>
 	</header>
 	<div class="module_content">
      <form method="post" name='input_rka' id='input_rka' action="<?php echo site_url('ppas/save_rka')?>" enctype="multipart/form-data" >
      <input name='call_from' type='hidden' id="call_from" value='<?php echo isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '' ?>'/>
        ...
        <table id="rka_input" class="fcari" width="100%">
          ...
          <div class="submit_link">
    			...
    			<input type="button" value="Keluar" onclick="window.location.href='<?php echo site_url('ppas'); ?>'" />
                        <input type="button" value="Batal" onclick="window.location='<?php
              	$call_from = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : '';
              	if(!empty($call_from) && strpos($call_from, 'ppas/cru_rka') == FALSE)
              		echo $call_from;
              	else
              		echo site_url('ppas/');
              ?>'"/>
