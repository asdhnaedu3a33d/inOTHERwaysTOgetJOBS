...
<script type="text/javascript">
	$(document).ready(function(){
		$(".veri_keg").click(function(){
	    	...
	    	$.ajax({
				type: "POST",
				url: "<?php echo site_url('ppas/do_veri'); ?>",
		...
		$("#kembali_all").click(function(){
			$(location).attr('href', '<?php echo site_url("ppas/veri_view"); ?>')
		});
		...
		$("#disapprove_renja").click(function(){
	    	...
	    	$.ajax({
				type: "POST",
				url: "<?php echo site_url('ppas/disapprove_renja'); ?>",
	...
	<div class="module_content";>
		<div style="overflow:auto">
			<table class="table-common" width="100%">
				...
				<?php
					foreach ($ppas as $row) {
						$indikator_program = $this->m_ppas->get_indikator_prog_keg($row->id, FALSE);
				...
				?>
					...
					<?php
						...
						$ppas_kegiatan = $this->m_ppas->get_all_kegiatan($row->id, $id_skpd, $th_anggaran);
						foreach ($ppas_kegiatan as $row2) {
							$indikator_kegiatan = $this->m_ppas->get_indikator_prog_keg($row2->id, FALSE);
