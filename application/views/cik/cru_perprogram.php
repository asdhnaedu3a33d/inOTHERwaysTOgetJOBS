...
<div style="width: 900px">
	...
	<div class="module_content">
		<form action="<?php echo site_url('cik/save_program_cik');?>" method="POST" name="kegiatan" id="kegiatan" accept-charset="UTF-8" enctype="multipart/form-data" >
			...
			<table class="fcari" width="100%">
				<tbody>
					...
					<tr>
                      ...
                      <td colspan="5"><?php echo form_dropdown('status_target['. $i .']', $status_indikator, $row->status_indikator, 'class="common status_target" id="status_target" disabled="disabled"'); ?></td>
                  	</tr>
										<tr>
											<td colspan="5"><?php echo form_dropdown('kategori_target['. $i .']', $kategori_indikator, $row->kategori_indikator, 'class="common indikator_val" id="kategori_target" disabled="disabled"'); ?></td>
										</tr>
										<tr style="width:100%">
											<td>Target</td>
                      ...
                      <td  bgcolor="#00FF33">Realisasi</td>
                      <td  bgcolor="#00FF33"><input style="width: 100%;" type="text" class="real" name="real_<?php echo $bulan;?>[<?php echo $i; ?>]" value="<?php echo (!empty($row->realisasi))?$row->realisasi:''; ?>"></td>
											<td style="width: 100px;"><input type="text" name="hasil" class="hasil" value="" id="hasil[<?php echo $i; ?>]" readonly="readonly"></td>
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		function hitung_cik(){
			var real, target, kinerja, tot_kinerja=0, count=0, capaian=0;
			$(".target").each(function(){
				count++;
				target = $(this).val();
				status = $(this).parent().parent().parent().find(".status_target").val();
				real = $(this).parent().parent().find(".real").val();
				if (real==''){
					real=0;
				};
				if (status == "1") {
					kinerja = ((real/target)*10000)/100;
					kinerja = kinerja.toFixed(2);
					tot_kinerja+=parseFloat(kinerja);
				}else {
					real = parseFloat(real);
					kinerja = ((target - real)+real)/real*100;
					kinerja = kinerja.toFixed(2);
					tot_kinerja+=parseFloat(kinerja);
				}
				$(this).parent().parent().find(".hasil").val(kinerja);

			});
			capaian = Math.round((tot_kinerja/count)*100)/100;
			$("#tot_kinerja").val(capaian);
		}
