<script type="text/javascript">
	...
	$(document).ready(function(){
	    dt = $("#pengendalian_table").DataTable({
            ...
            "ajax": {
	            "url": "<?php echo base_url('kendali_belanja_perubahan/load_kendali'); ?>",
	            "type": "POST"
	        }
	    });
	...
	function edit_kendali_belanja(id){
		window.location = '<?php echo base_url('kendali_belanja_perubahan/edit_kendali_belanja/');?>' + '/' + id;
	}

	function delete_kendali_belanja(id){
		if (confirm('Apakah anda yakin untuk menghapus data pengendalian belanja perubahan ini?')) {
			$.blockUI({
...
<article class="module width_full">
	<header>
		<h3> FORMAT KENDALI PELAKSANAAN KEGIATAN BELANJA LANGSUNG PERUBAHAN </h3>
	</header>
	<div style='float:right'><a href="<?php echo site_url('kendali_belanja_perubahan/cru_kendali_belanja')?>">Tambah Data Kendali Belanja Perubahan</a></div>
	<h3>
	<div>SKPD   : </div>
	<div>URUSAN : </div>
	</h3>
