<div align="center">Tabel Rencana Aksi Kegiatan</div>
<table border="1" style="border-collapse: collapse"  width="100%">
	...
	<tbody>
		<?php  
			$i = 1;
			$aksi = (array)$aksi;
			$kegiatan = $this->m_dpa_perubahan->get_all_kegiatan($aksi['id'], $aksi['id_skpd'], $aksi['tahun']);
		?>
		...
		<?php 
			...
			foreach ($kegiatan as $key => $row) { 
				...
				$indikator = $this->m_dpa_perubahan->get_indikator_prog_keg($row->id, true, true);
				...
				foreach ($rencana_aksi as $key2 => $row2) {
					 ...
					 $sum_anggaran_aksi = $this->m_dpa_perubahan->sum_anggaran_rencana_aksi($row2->id_dpa_prog_keg,$row2->bulan)->row();
					 $realisasi = $this->m_dpa_perubahan->get_realisasi_dari_triwulan($row2->id_dpa_prog_keg,$row2->bulan)->row();
